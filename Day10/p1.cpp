...
public:
    ...
    char at(const Vector2D& pos) const {
        return grid[pos.x*width + pos.y];
    }
...
std::unordered_set<int> get_num_trails(const Grid& grid, Vector2D pos) {
    if(grid.at(pos) == '9') {
        return {grid.get_index(pos)};
    }
    std::unordered_set<int> num_trails {};
    Vector2D new_pos = pos + Vector2D(1, 0);
    if (grid.inBounds(new_pos) && grid.at(new_pos) - grid.at(pos) == 1) {
        num_trails.merge(get_num_trails(grid, new_pos));
    }
...
int main(int argc, char** argv) {
    ...
    // auto num_trails = get_num_trails(map, trailheads[0]);
    // std::println("{} ", num_trails.size());
    size_t num_trails_tot = 0;
    for(const auto& elem: trailheads) {
        auto num_trails = get_num_trails(map, elem);
        num_trails_tot += num_trails.size();
        // std::println("{} ", num_trails);
    }
